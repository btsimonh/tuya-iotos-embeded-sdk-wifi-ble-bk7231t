...
void udp_server_thread( beken_thread_arg_t arg )
{
	...
	int udp_listen_fd = -1;
      ...
      PR_NOTICE( "udp_server_thread ucalled\n" );
  ...
  //  gbuf = buf;
    ...
    server_addr.sin_port = htons(UDP_LISTENER_PORT);
      ...
      PR_NOTICE( "udp_server_thread udp_listen_fd: %d\n",udp_listen_fd );
    ...
    while ( 1 )
    {
      ...
      PR_NOTICE( "udp_server_thread udp_listen_fd: %d before select\n",udp_listen_fd );
        select( udp_listen_fd + 1, &readfds, NULL, NULL, NULL);

      PR_NOTICE( "udp_server_thread udp_listen_fd: %d after select\n",udp_listen_fd );
        if ( FD_ISSET( udp_listen_fd, &readfds ) )
        {
            if ( FD_ISSET( udp_listen_fd, &readfds ) ) //one client has data
            {
                ...
                if(len)
                {
      PR_NOTICE( "udp_server_thread udp_listen_fd: %d has received %s\n",udp_listen_fd,buf );
					strcpy(buf,"Hello reply 2");
					len = strlen(buf)+1;
...
exit:
    ...
    if(buf)
    {
      ...
      PR_NOTICE( "udp_server_thread rtos_delete_thread" );
...
OPERATE_RET device_init(VOID)
{
	...
	myInit();
	connect_to_wifi("ssidd","fdhs5w4t34qfsdfdsaa");
	demo_start_upd();
